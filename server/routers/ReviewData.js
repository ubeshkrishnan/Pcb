...
app.put('/reviewdata/:sample_id', (req, res) => {
...
// Check if sample_id is a valid integer
  ...
  db.query(
    `
    SELECT SC.ref_id, ST.schedule_type, SD.serial_no, SD.collection_time, SD.sampled_by, SD.latitude, SD.longitude, SD.sample_type_id, SD.poc_id, SD.container_type, SD.sample_color, SD.sample_turbidity, SD.treatment_type, SD.sample_photos
...
// // Create a multer middleware to handle FormData
// const upload = multer().none();

// app.put('/reviewdata/:sample_id', upload, (req, res) => {
//   const sample_id = parseInt(req.params.sample_id);
...
// app.post('/reviewdata/:sample_id', (req, res) => {
//   // Retrieve data from the database using a query with JOIN
//   const sample_id = req.params.sample_id;

//   db.query(`
//     SELECT SC.ref_id,ST.schedule_type,SD.serial_no,SD.collection_time,U.employee_id,SD.latitude, SD.longitude,SD.sample_type_id,SD.poc_id,SD.container_type,SD.sample_color,SD.sample_turbidity,SD.treatment_type,SD.sample_photos
//     FROM sample_details AS SD
//     JOIN sample_collection AS SC ON SD.sample_coll_id = SC.sample_coll_id
//     JOIN m_schedule_type AS ST ON SC.schedule_type = ST.schedule_type_id
//     JOIN users AS U ON U.user_id = SD.created_by
//     WHERE ST.schedule_type_id = 1
//     AND SD.sample_id = 1;`, (error, result) => {
//     if (error) {
//       console.error('Error executing query:', error);
//       res.status(500).json({ error: 'Internal server error' });
//     } else {
